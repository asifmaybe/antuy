...
import { LogOut, ClipboardList, GraduationCap, Users, Bell, BarChart3, Clock, Activity } from "lucide-react";
import { LanguageSwitcher } from "@/components/LanguageSwitcher";
import { useLanguage } from "@/hooks/use-language";
import { mockAdminAssignments, mockAdminExams, mockAdminResults, mockAuditLog } from "@/lib/admin-data";
...
function AdminDashboard() {
  ...
  const user = (() => {
    try {
      return JSON.parse(sessionStorage.getItem("eduportal_user") || "{}");
    } catch { return {}; }
  })();

  const activeAssignments = mockAdminAssignments.filter(a => a.status === "active").length;
  const overdueAssignments = mockAdminAssignments.filter(a => a.status === "overdue").length;
  const upcomingExams = mockAdminExams.filter(e => e.upcoming).length;
  const totalResults = mockAdminResults.length;
