...
function ExamsPage() {
  ...
  const upcoming = mockExams.filter((e) => e.upcoming);
  const past = mockExams.filter((e) => !e.upcoming);
              ...
              {upcoming.map((exam) => (
                <div key={exam.id} className="rounded-xl border bg-card p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-accent px-2.5 py-1 text-xs font-medium text-accent-foreground">{exam.type}</span>
                  </div>
                  <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                    <span>{t("date")}: <strong className="text-foreground">{exam.date}</strong></span>
                    <span>{t("marks")}: <strong className="text-foreground">{exam.marks}</strong></span>
                  </div>
                  {exam.instructions && <p className="text-xs text-muted-foreground mt-2 italic">{exam.instructions}</p>}
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="space-y-3">
              {past.map((exam) => (
                <div key={exam.id} className="rounded-xl border bg-card p-4 opacity-70">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-sm">{exam.subject}</h3>
                    <span className="rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">{exam.type}</span>
                  </div>
                  <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                    <span>{t("date")}: {exam.date}</span>
                    <span>{t("marks")}: {exam.marks}</span>
                  </div>
                </div>
              ))}
            </div>
