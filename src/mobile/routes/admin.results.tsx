...
import { fetchAllResults, fetchStudentsList, createResult, logAuditEntry, type Result } from "@/lib/api";
...
function AdminResultsPage() {
  ...
  const [form, setForm] = useState({ studentId: "", subject: "", examType: "Quiz" as Result["exam_type"], marks: "", totalMarks: "", date: "" });
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <select
                    value={form.examType}
                    onChange={e => setForm(f => ({ ...f, examType: e.target.value as Result["exam_type"] }))}
                    className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm"
                  >
              ...
              <div key={r.id} className="rounded-xl border bg-card p-3.5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{r.student_id} · {r.subject} · {r.exam_type}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-primary">{r.marks}<span className="text-xs text-muted-foreground font-normal">/{r.total_marks}</span></p>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground mt-1.5">
                  {lang === "bn" ? "আপলোড করেছেন" : "Uploaded by"} {r.uploaded_by} · {formatDateTime(r.uploaded_at)}
                </p>
              </div>
            ))}
          </div>
