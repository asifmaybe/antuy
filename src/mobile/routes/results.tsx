              ...
              <div key={result.id} className="rounded-xl border bg-card p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{result.exam_type} · {formatDate(result.date)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-primary">{result.marks}<span className="text-xs font-normal text-muted-foreground">/{result.total_marks}</span></p>
