      ...
      <main className="px-4 space-y-4">
            ...
            {isProcessed && (
              <div className="space-y-3">
                ...
                <div className="rounded-xl border bg-card overflow-hidden">
                  <div className="grid grid-cols-[2rem_1fr_5.5rem_2.5rem] gap-1 px-2 py-2 bg-muted/50 text-[9px] font-semibold text-muted-foreground uppercase tracking-wide">
                    <span>#</span>
                    <span>{lang === "bn" ? "নাম" : "Name"}</span>
                    <span className="pl-10">{lang === "bn" ? "রোল" : "Roll"}</span>
                    <span className="text-center">{lang === "bn" ? "স্থিতি" : ""}</span>
                  </div>
                  ...
                  <div style={{ maxHeight: "350px", overflowY: "auto" }}>
                    {records.map((r, i) => (
                      <div
                        key={r.studentId}
                        className={`grid grid-cols-[2rem_1fr_5.5rem_2.5rem] gap-1 items-center px-2 py-1.5 text-xs border-t ${r.status === "absent" ? "bg-destructive/5" : ""
                          }`}
                      >
                        <span className="text-muted-foreground text-[10px]">{i + 1}</span>
                        <span className="truncate font-medium text-[11px]">{r.studentName}</span>
                        <span className="font-mono text-[12px] font-bold pl-4">{r.rollNumber}</span>
                        <button
                          onClick={() => toggleStatus(r.studentId)}
                          className={`w-7 h-5 mx-auto flex items-center justify-center rounded text-[10px] font-bold ${statusStyle(r.status)}`}
                        >
                ...
                <p className="text-[10px] text-muted-foreground text-center">
                  {lang === "bn"
                    ? "স্থিতি বাটনে ক্লিক করে পরিবর্তন করুন"
                    : "Tap status to toggle: ✓ → ✗ → L"}
                </p>
        ...
        <section>
          ...
          <p className="text-[10px] text-muted-foreground mb-2">
            {lang === "bn" ? "বিস্তারিত দেখতে ট্যাপ করুন" : "Tap a session to view details"}
          </p>
          {sessionsLoading ? (
                  ...
                  <div key={s.id} className="rounded-xl border bg-card overflow-hidden">
                    ...
                    {isExpanded && s.records && s.records.length > 0 && (
                      <div className="border-t">
                        <div className="grid grid-cols-[2rem_1fr_5.5rem_2.5rem] gap-1 px-2 py-1.5 bg-muted/50 text-[9px] font-semibold text-muted-foreground uppercase tracking-wide">
                          <span>#</span>
                          <span>{lang === "bn" ? "নাম" : "Name"}</span>
                          <span className="pl-4">{lang === "bn" ? "রোল" : "Roll"}</span>
                          <span className="text-center">{lang === "bn" ? "স্থিতি" : ""}</span>
                        </div>
                        <div style={{ maxHeight: "350px", overflowY: "auto" }}>
                          {s.records.map((rec, idx) => {
                            ...
                            return (
                              <div
                                key={rec.id}
                                className={`grid grid-cols-[2rem_1fr_5.5rem_2.5rem] gap-1 items-center px-2 py-1.5 text-xs border-t ${rec.status === "absent" ? "bg-destructive/5" : ""}`}
                              >
                                <span className="text-muted-foreground text-[10px]">{idx + 1}</span>
                                <span className="truncate font-medium text-[11px]">{rec.student_name}</span>
                                <span className="font-mono text-[12px] font-bold pl-4">{stu?.roll_number ?? "—"}</span>
