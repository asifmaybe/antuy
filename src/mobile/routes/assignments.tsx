...
function AssignmentsPage() {
  ...
  const statusConfig = {
    ...
    completed: { label: t("submitted"), icon: CheckCircle2, className: "text-success bg-success/10" },
  };
            ...
            return (
              <div key={assignment.id} className="rounded-xl border bg-card p-4">
                ...
                <div className="flex gap-4 mt-3 text-xs text-muted-foreground">
                  <span>{t("assigned")}: <strong className="text-foreground">{assignedDate}</strong></span>
                  <span>{t("due")}: <strong className="text-foreground">{dueDate}</strong></span>
                </div>
              </div>
            );
          })
